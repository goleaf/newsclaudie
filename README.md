...
Comprehensive documentation is available in the `docs/` directory, grouped by function (for example `docs/admin`, `docs/comments`, `docs/news`, and `docs/design-tokens`). Run `npm run docs:verify` to move stray Markdown into the correct docs folders, rewrite links, check for broken doc links, and ensure the tree stays consistent.
