 ...
 */
final class MarkdownFileParser extends Controller
{
    /**
    ...
    public static function getMarkdownPostsAsArray(): array
    {
        ...
        foreach (glob(self::getQualifiedFilepath('*', false)) as $filepath) {
            $files[mb_substr(basename($filepath), 0, -3)] = $filepath; // Remove the extension and add the filepath to the array
        }
     ...
     * @return $this
     */
    public function parse(string $filename, ?string $filepath = null)
    {
        if (! $filepath) {
            $filepath = self::getQualifiedFilepath($filename);
        }
    ...
    private function validateMatter(array $matter): array
    {
        $validator = Validator::make($matter, [
            ...
            'published' => 'required|date_format:Y-m-d H:i',
        ...
        $author = User::find($validated['author']);
